function [eigenvectors,eigenvalues] = compute_eigenvectors_sliding_cov(timeseries,half_window_size,n_eigen) 

% here I compute covariance matrices using a sliding window. Then
...
for t=1:T-2*half_window_size
    ...
    demeaned_truncated = truncated_timeseries-mean(truncated_timeseries); % as you can see from the formula (***) of the paper, 
                                                      % corr starts from
                                                      % summing the
                                                      % zscored
    normalizing_factor = size(truncated_timeseries,1)-1;                                              
    demeaned_truncated = (1/sqrt(normalizing_factor))*demeaned_truncated;
    ...
    % the covariance matrix is zscored_truncated'*zscored_truncated.
    % However, instead of running an expensive EVD on it, now we can use
    % our formula
                                                     
    minimatrix = demeaned_truncated*demeaned_truncated';
    ...
    eigenvectors(:,:,t) = demeaned_truncated'*v;
